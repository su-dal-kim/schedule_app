...
Supabase(저장) + Google Gemini(무료 사진 분석, Gemini 3.8 Flash) + Vercel(호스팅) 구조입니다.
...
3. 왼쪽 메뉴 **Project Settings → API** 에서 다음 두 값을 복사해두기
   ...
   - **API Keys** 탭 → **Publishable and secret API keys** 하위의 `Publishable key` (예: `sb_publishable_...`로 시작) → `VITE_SUPABASE_PUBLISHABLE_KEY`
   (예전에는 이 키를 "anon public key"라고 불렀어요. 지금은 이름만 바뀐 거고 역할은 같아요.)
...
## 참고사항
- 이 구성은 로그인 기능이 없는 개인용 버전이에요. Supabase의 Publishable key와 URL을 아는 사람은 누구나 이 표에 접근할 수 있어요(민감한 정보가 아니라면 개인 사용엔 문제없는 수준이에요).
