...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data: rows, error } = await supabase.from("schedules").select("key,label,data");
        if (error) throw error;
        if (rows) setHistory(sortHistory(rows));
      } catch (e) {
        console.error("저장된 기록을 불러오지 못했어요:", e.message || e);
      }
    })();
  }, []);
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      try {
        const { error } = await supabase.from("schedules").upsert({ key: safeKey, label, data: parsed });
        if (error) throw error;
      } catch (saveErr) {
        console.error("Supabase 저장 실패:", saveErr.message || saveErr);
      }
    } catch (e) {
      setStatus("error");
